#!/bin/bash
#$ -cwd
cd /shelf/apps/pjt6/Hsh/blobs/default

conda activate python27


minialign -t12 -xont Hs_default_trimmed.ctg._L5000.lay.fa /shelf/apps/pjt6/Hsh/Hsac_all_data_reduce_ploidy_300X_CMAPSEN_normal.trimmedReads.fasta.gz > temp.sam
wait 
...
#purge_haplotigs purge  -g Hs_default_trimmed.ctg._L5000.lay.fa  -c coverage_stats.csv  -b sorted.bam  -t 18  -a 60

#cp curated.haplotigs.fasta reassigned.haplotigs.fasta


#python ~/Downloads/blobtools-light-master/mapping2cov.py -a /home/pt40963/scratch/nematode/newton/purge_haplo_ALL_haplotypes_all_data/purge_the_purged/Hs_default_trimmed.ctg._L5000.lay.fa -bam /home/pt40963/scratch/nematode/newton/purge_haplo_ALL_haplotypes_all_data/purge_the_purged/sorted.bam -o contig.mappingall_final.cas.cov


######################################


blastn -task megablast -query Hs_default_trimmed.ctg._L5000.lay.fa -db nt -outfmt '6 qseqid staxids bitscore std scomnames sscinames sblastnames sskingdoms stitle' -evalue 1e-20 -out n.clc.allfinal.out -num_threads 8

blobtools create -i Hs_default_trimmed.ctg._L5000.lay.fa  -s temp.sam -t n.clc.allfinal.out -o Hs_default_trimmed.ctg._L5000.lay.fa.blobplots
 
 mkdir allfinal.fa.blobplots
 cp Hs_default_trimmed.ctg._L5000.lay.fa.blobplots.blobDB.json ./allfinal.fa.blobplots
 
blobtools view -i  Hs_default_trimmed.ctg._L5000.lay.fa.blobplots.blobDB.json -o allfinal.fa.blobplots/
 
blobtools blobplot -i Hs_default_trimmed.ctg._L5000.lay.fa.blobplots.blobDB.json -o allfinal.fa.blobplots/ 
...
python /shelf/apps/pjt6/apps/public_scripts-master/get_sequences_i_want_from_fasta_command_line_not_wanted_file.py Hs_default_trimmed.ctg._L5000.lay.fa ./allfinal.fa.blobplots/bad_contigs.out 10 Hs_default_trimmed.ctg._L5000.lay.contim_filtered.fasta
rm temp.sam
